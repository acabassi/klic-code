...
for(i in 1:n_types){
    ...
    cophenetic_6clusters[i] <- copheneticCorrelation(CM_temp)
}
...
for(i in 1:n_types){
  # Use consensus clustering to find kernel matrix
  CM_temp <- consensusCluster(data[,,i,j], n_clusters, clMethod = "hclust")
  # Shift the eigenvalues of the kernel matrix so that it is positive
  # semi-definite
  CM_temp <- spectrumShift(CM_temp)
  # Use kernel k-means to find clusters
  ...
  cophenetic_3clusters[i] <- copheneticCorrelation(CM_temp)
}
...
save(
  ...
  file = paste0("../results/ari-c-", j,"-new.RData"))
