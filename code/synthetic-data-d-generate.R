...
for(i in 1:n_datasets_same_rho){
  for(j in 1:n_experiments){
    ...
    for(k in 1:n_clusters){
      mu = c(rep((k*(separation_level-1)/2), 2),
             rep(0, 13))
...
ggsave(paste0("../figures/synthetic-data-d-sep", separation_level, ".pdf"),
       width = 7, height = 7)

save(data, file =
       paste0("../data/synthetic-data-d-sep", separation_level,".RData"))
